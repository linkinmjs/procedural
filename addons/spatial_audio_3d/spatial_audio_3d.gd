...
@export var output_bus: StringName = &"Master" ## Bus al que envian los buses generados (fuente, reverbs). Parche local: el original enviaba siempre a Master y se salteaba el volumen de SFX.
...
func _ready():
	...
	# @export vars
	soundsource.output_bus = output_bus
...
func create_soundplayer(name_p: String, with_reverb_fx: bool = true) -> Soundplayer:
	...
	# @export vars
	soundplayer.output_bus = output_bus
...
func create_audiobus(bus_name, vol_db = 0):
	...
	AudioServer.set_bus_send(a, output_bus if AudioServer.get_bus_index(output_bus) >= 0 else &"Master")
...
class Soundsource extends SpatialAudio3D:
	...
	func create_reverber(name_p, raycast_index: int) -> Reverber:
		...
		# @export vars
		reverber.output_bus = output_bus
		reverber.loop = loop
